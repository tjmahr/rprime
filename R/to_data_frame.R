...
#'   EprimeFrames)
#' @param ... other parameters passed onto \code{as.data.frame}
...
to_data_frame.default <- function(x, ...) {
  data_frames <- lapply(x, as.data.frame, stringsAsFactors = FALSE, ...)
...
#' @export
as.data.frame.FrameList <- to_data_frame.FrameList

#' @export
as.data.frame.EprimeFrame <- to_data_frame.EprimeFrame

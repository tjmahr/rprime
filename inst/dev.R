...
flatten_chunks <- function(eprime_lists) {

  set_chunk_number <- function(chunk, number) {
    chunk[["Eprime.ChunkNumber"]] <- number
    chunk
  }


  eprime_lists <- Map(set_chunk_number, eprime_lists, seq_along(eprime_lists))
...
#'
#' @param filename a path to a .txt file generated by Eprime
...
#'   is written in the same directory as the filename fed into this function.
ProcessFile <- function(filename, trial_level) {
  require(tools)
  require(plyr)
  require(stringr)
  eprime_log <- load_eprime(filename)
  file_basename <- file_path_sans_ext(filename)
...
clean_up_header <- function(eprime_log) {
  ...
  exp_line <- str_which(header, "Experiment:")
  clock_line <- str_which(header, "Clock.Information:")
  after_last_level <- max(str_which(header, "LevelName: ")) + 1
